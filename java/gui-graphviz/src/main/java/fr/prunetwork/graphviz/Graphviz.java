...
public class Graphviz {
     ...
     */
    public static int writeGraphToFile(byte[] img, String file) {
    ...
    private static byte[] get_img_stream(File dot, GraphvizExportType type, GraphvizRenderer renderer) {
        ...
        try {
            File img = File.createTempFile("graph_", "." + type, new File(Graphviz.TEMP_DIR));
            Runtime rt = Runtime.getRuntime();

            // patch by Mike Chenault
            String[] args = {renderer.getPath(), "-T" + type, dot.getAbsolutePath(), "-o", img.getAbsolutePath()};
    ...
    public byte[] getGraph(GraphvizExportType type, GraphvizRenderer renderer) {
        ...
        dot = writeDotSourceToFile(getDotSource());
        if (dot != null) {
            img_stream = get_img_stream(dot, type, renderer);
            if (dot.delete() == false) {
